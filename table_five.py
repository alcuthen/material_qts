import time 
from selenium import webdriver
from selenium.webdriver.common.by import By


browser: object = webdriver.Chrome() 

browser.get("https://www.saucedemo.com/")

# Extrair o título da página web pelo atributo title do objeto webdriver
title_page: str = browser.title
print(title_page)

try:
    if(title_page == "Swag Labs" and browser.current_url == 'https://www.saucedemo.com/'):
        print(f"A página {title_page} está correta!")

        browser.maximize_window()
      
        username: object = browser.find_element(By.ID, "user-name")
        password: object = browser.find_element(By.ID, "password")
        btn_log_user: object = browser.find_element(By.ID, "login-button")

        username.send_keys("standard_user")
        password.send_keys("secret_sauce")
        btn_log_user.click()

        product_title = browser.find_element(By.XPATH, "//span[@class='title']")

        time.sleep(3)
        
        if product_title.text == "Products":
            print("Encontrou o elemento na página")
        


except NameError:
    print(f"O acesso a página {title_page} não foi possível {NameError}")

finally:
    browser.quit()
